...
final class GrabDabAppDelegate: NSObject, NSApplicationDelegate {
    private let settings = UserSettings.shared
    private lazy var statusItemController = StatusItemController()
    private lazy var captureController = CaptureController(settings: settings)
    private lazy var toastPresenter = ToastPresenter.shared
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(reprocessLastCapture),
            name: .didPurchaseUpgrade,
            object: nil
        )
    }
    ...
    func showUpgradeSheet(context: UpgradeContext) {
        // Close existing upgrade window if any
        upgradeWindow?.close()
        upgradeWindow = nil

        let upgradeView = UpgradeView(context: context)
        let hostingView = NSHostingView(rootView: upgradeView)

        upgradeWindow = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: 480, height: 600),
            styleMask: [.titled, .closable],
            backing: .buffered,
            defer: false
        )
        upgradeWindow?.title = "Upgrade to GrabDab Pro"
        upgradeWindow?.contentView = hostingView
        upgradeWindow?.center()
        upgradeWindow?.level = .floating
        upgradeWindow?.isReleasedWhenClosed = false
        upgradeWindow?.makeKeyAndOrderFront(nil)

        NSApp.activate(ignoringOtherApps: true)
    }

    @objc private func reprocessLastCapture() {
        print("Reprocessing last capture after upgrade...")
        captureController.reprocessLastCapture { [weak self] result in
            guard let self = self else { return }
            switch result {
            case let .success(ocrResult):
                // The UpgradeView is presented via a sheet, which is managed by SwiftUI.
                // We can't easily close it from here. However, the view that presented it
                // should be responsible for dismissing it. The purchase success should
                // also dismiss the sheet.
                self.showCaptureResult(ocrResult)
            case .failure(let error):
                // Don't show an error if reprocessing fails, it's a bonus action.
                print("Failed to reprocess last capture: \(error.localizedDescription)")
            }
        }
    }
}
